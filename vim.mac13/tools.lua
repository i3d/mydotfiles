...
function tools.gunzip(infile, outfile)
   if infile:find(".tar.gz") then
      fs.execute(fs.Q("tar").." xvfz "..fs.Q(infile))
      if infile:find("say") then
         fs.execute(fs.Q("tar").." cvfz "..fs.Q(infile).." "..fs.Q(infile:gsub("v1.3.*1.tar.gz", "say-1.3-1")))
         return uncompress("gz", "gunzip", infile, outfile)
      end

      if infile:find("luassert") then
         fs.execute(fs.Q("tar").." cvfz "..fs.Q(infile).." "..fs.Q(infile:gsub("v1.8.0.tar.gz", "luassert-1.8.0")))
         return uncompress("gz", "gunzip", infile, outfile)
      end

      if infile:find("term") then
         fs.execute(fs.Q("tar").." cvfz "..fs.Q(infile).." "..fs.Q(infile:gsub("0.07.tar.gz", "lua-term-0.07")))
         return uncompress("gz", "gunzip", infile, outfile)
      end
      if infile:find("mediator") then
         fs.execute(fs.Q("tar").." cvfz "..fs.Q(infile).." "..fs.Q(infile:gsub("v1.1.2.*0.tar.gz", "mediator_lua-1.1.2-0")))
         return uncompress("gz", "gunzip", infile, outfile)
      end
      if infile:find("busted") then
         fs.execute(fs.Q("tar").." cvfz "..fs.Q(infile).." "..fs.Q(infile:gsub("v2.0.0.tar.gz", "busted-2.0.0")))
         return uncompress("gz", "gunzip", infile, outfile)
      end
      if infile:find("cliargs") then
         fs.execute(fs.Q("tar").." cvfz "..fs.Q(infile).." "..fs.Q(infile:gsub("v3.0.*2.tar.gz", "lua_cliargs-3.0-2")))
         return uncompress("gz", "gunzip", infile, outfile)
      end
      if infile:find("luasystem") then
         fs.execute(fs.Q("tar").." cvfz "..fs.Q(infile).." "..fs.Q(infile:gsub("v0.2.1.tar.gz", "luasystem-0.2.1")))
         return uncompress("gz", "gunzip", infile, outfile)
      end
      if infile:find("client") then
         fs.execute(fs.Q("tar").." cvfz "..fs.Q(infile).." "..fs.Q(infile:gsub("/0.2.2.*1.tar.gz", "/lua-client-0.2.2-1")))
         return uncompress("gz", "gunzip", infile, outfile)
      end
      fs.execute(fs.Q("tar").." cvfz "..fs.Q(infile).." "..fs.Q(infile:gsub(".tar.gz", "")))
   end
